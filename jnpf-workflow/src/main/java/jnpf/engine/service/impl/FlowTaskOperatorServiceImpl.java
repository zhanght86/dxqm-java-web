...
public class FlowTaskOperatorServiceImpl extends ServiceImpl<FlowTaskOperatorMapper, FlowTaskOperatorEntity> implements FlowTaskOperatorService {
    ...
    public void update(String taskNodeId, List<String> userId,String completion) {
        if (userId.size() > 0) {
            this.baseMapper.updateDelegateUser(taskNodeId, "'" + String.join("','", userId) + "'",completion);
        }
    }
    ...
    @Override
    public void updateReject(String taskId, Set<String> nodeCode) {
        if (nodeCode.size() > 0) {
            this.baseMapper.updateReject(taskId, "'" + String.join("','", nodeCode) + "'");
        }
    }
