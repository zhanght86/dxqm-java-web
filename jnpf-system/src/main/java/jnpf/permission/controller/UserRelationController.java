...
import jnpf.permission.model.userrelation.UserRelationListVO;
...
//    }
    ...
    @GetMapping("/{objectId}" )
    public ActionResult<ListVO<UserRelationListVO>> listTree(@PathVariable("objectId" ) String objectId) {
        List<UserRelationEntity> data = userRelationService.getListByObjectId(objectId);
            ...
            for (String userId : userRelationForm.getUserIds()) {
                ...
                if (userEntity != null) {
                    ...
                    if ("Position".equals(userRelationForm.getObjectType())) {
                        //更新用户岗位id
                        String originalId = userEntity.getPositionId();
                        if(StringUtil.isNotNull(originalId)) {
