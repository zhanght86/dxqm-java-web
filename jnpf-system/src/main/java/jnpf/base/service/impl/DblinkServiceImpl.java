...
public class DblinkServiceImpl extends ServiceImpl<DbLinkMapper, DbLinkEntity> implements DblinkService {
    ...
    public List<DbLinkEntity> getList(String keyWord) {
        ...
        queryWrapper.lambda().orderByAsc(DbLinkEntity::getSortCode).orderByDesc(DbLinkEntity::getCreatorTime);
        return this.list(queryWrapper);
    }
    ...
    public boolean testDbConnection(DbLinkEntity entity) throws DataException{
        ...
        if(db==null){
            throw DataException.DbTypeCompare("连接中，");
        }
        Connection conn = JdbcUtil.getConn(entity);
        return conn != null;
    }
