...
public class DownUtil {
    ...
    public static void dowloadFile(File file) {
        try {
            ...
            if (-1 != agent.indexOf("msie") || -1 != agent.indexOf("trident")) {
                //IE
                codeFileName = URLEncoder.encode(file.getName(), Constants.UTF_8);
            } else if (-1 != agent.indexOf("mozilla")) {
                //火狐，谷歌
                codeFileName = new String(file.getName().getBytes(Constants.UTF_8), StandardCharsets.ISO_8859_1);
            } else {
                codeFileName = URLEncoder.encode(file.getName(), Constants.UTF_8);
            }
     ...
     * 显示预览的pdf
     */
    public static void dowloadFileHtml(File file) {
        try {
            @Cleanup OutputStream outputStream = null;
            @Cleanup InputStream in = null;
            //读取指定路径下面的文件
...
//            ServletUtil.getResponse().setHeader("Content-Disposition", "filename=" + codeFileName);
            ServletUtil.getResponse().setHeader("X-Frame-Options", "SAMEORIGIN");
            outputStream = new BufferedOutputStream(ServletUtil.getResponse().getOutputStream());
            //创建存放文件内容的数组
            byte[] buff = new byte[1024];
            //所读取的内容使用n来接收
    ...
    public static void dowloadSvgFile(File file) {
        try {
            ...
            if (-1 != agent.indexOf("msie") || -1 != agent.indexOf("trident")) {
                ...
                //火狐，谷歌
                codeFileName = new String(file.getName().getBytes(Constants.UTF_8), "iso-8859-1");
            } else {
                codeFileName = URLEncoder.encode(file.getName(), Constants.UTF_8);
            }
            ServletUtil.getResponse().setHeader("Content-Disposition", "filename=" + codeFileName);
