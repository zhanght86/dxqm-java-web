...
import jnpf.onlinedev.model.KeyDataConst;
...
public class KeyDataUtil {
     ...
     * @throws SQLException
     */
    public static List<VisualdevModelDataEntity> getHasTableList(List<VisualdevModelDataEntity> list, String mainTable, List<ColumnListField> modelList, ColumnDataModel columnData, DbLinkEntity linkEntity) throws DataException, SQLException {
    ...
    public static List<Map<String, Object>> changeGroupDataList(ColumnDataModel columnDataModel, List<Map<String, Object>> realList) {
        ...
        //用来转换空值的List
        List<Map<String, Object>> nullList = new ArrayList<>(realList.stream().filter(t -> "null".equals(String.valueOf(t.get(columnDataModel.getGroupField())))).collect(Collectors.toList()));
        Map<String, Object> nullMap = new HashMap<>(16);
        nullMap.put(firstField, "");
        nullMap.put("top", true);
        nullMap.put("id","null");
        nullMap.put("children", nullList);
        resultList.add(nullMap);
        if (nullList.size()==0){
            resultList.remove(nullMap);
        }
        List<String> groupField = realList.stream().map(t -> String.valueOf(t.get(columnDataModel.getGroupField()))).collect(Collectors.toList());
        HashSet<String> parents = new HashSet<String>(groupField);
        parents.remove("null");
        for (String parent : parents) {
            Map<String, Object> dataMap = new HashMap<>(16);
            List<Map<String, Object>> resultMapOneList = realList.stream().filter(t -> parent.equals(t.get(columnDataModel.getGroupField()))).collect(Collectors.toList());
     ...
     */
    public static Map<String, Object> getKeyData(List<FieLdsModel> formData, Map<String, Object> keyJsonMap, List<VisualdevModelDataEntity> list, String visualDevId,Integer method) throws IOException, ParseException, DataException, SQLException {
        init();
        //创建时间类关键词查询对应的字段实体
                        ...
                        if (entry.getKey().equals(field)) {
                            ...
                            switch (type) {
                                ...
                                case JnpfKeyConsts.ADDRESS:
                                    provKeyList.add(String.valueOf(entry.getValue()));
                                    break;
                                //关联表单
                            ...
                            switch (type) {
                                ...
                                case JnpfKeyConsts.ADDRESS:
                                    ...
                                    if (provKeyMap.containsKey(str)) {
                                        dataMap.put(field, provKeyMap.get(str));
                                    }
                                    break;
                                //时间范围
                                case JnpfKeyConsts.TIMERANGE:
                                    JSONArray jsonArrayTimeRange = JsonUtil.getJsonToJsonArray(String.valueOf(dataMap.get(field)));
                                    jsonArrayTimeRange = DateUtil.addCon(jsonArrayTimeRange, JnpfKeyConsts.TIMERANGE, "HH:mm:ss");
                                    dataMap.put(field, jsonArrayTimeRange);
                                    timeControl.setTimeRange(timeControl.getTimeRange() + field);
                                    break;
                                //时间选择
                                case JnpfKeyConsts.TIME:
                                    timeControl.setTime(timeControl.getTimeRange() + field);
                                    break;
                                //日期选择
                                case JnpfKeyConsts.DATE:
                                    String dateStr=String.valueOf(dataMap.get(field));
                                    List<String> dataAll = new ArrayList<>();
                                    if (dateStr.contains("[") || dateStr.contains("]")) {
                                        List<String> data = JsonUtil.getJsonToList(dateStr, String.class);
                                        dataAll.addAll(data);
                                    }else {
                                        dataAll.add(dateStr);
                                    }
                                    List<String> result = new ArrayList<>();
                                    for(String date : dataAll) {
                                        try {
                                            long time;
                                    ...
                                    dataMap.put(field, String.join(",",result));
                                    break;
                                 //创建时间
                                case JnpfKeyConsts.CREATETIME:
                                    timeControl.setCreateTime(timeControl.getCreateTime()+field);
                                    break;
                                //修改时间
                                case JnpfKeyConsts.MODIFYTIME:
                                    timeControl.setModifyTime(timeControl.getModifyTime() + field);
                                    break;
                                //日期范围
                                case JnpfKeyConsts.DATERANGE:
                                    if(String.valueOf(dataMap.get(field)).contains(",")){
                                ...
                                case JnpfKeyConsts.POPUPSELECT:
                                    ...
                                    Object data = dataInterfaceService.infoToId(model.getInterfaceId()).getData();
                                    List<Map<String, Object>> mapList =(List<Map<String, Object>>) data;
                                    for (Map<String,Object> map :mapList){
                                        if (map.get(model.getPropsValue()).equals(entry.getValue())){
                                            //列表默认取第一个字段的值
                                            String s = model.getColumnOptions().get(0).getValue();
