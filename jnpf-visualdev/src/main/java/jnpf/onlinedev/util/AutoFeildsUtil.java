                                ...
                                if (childFieLdsModel.getVModel().equals(data.getKey())) {
                                    ...
                                    switch (jnpfKeyType) {
                                        case JnpfKeyConsts.CREATEUSER:
                                            ...
                                            UserEntity entity = userEntityList.stream().filter(user -> user.getRealName().equals(String.valueOf(data.getValue()))).findFirst().orElse(null);
                                            data.setValue(entity.getId());
                                            break;
                                        case JnpfKeyConsts.MODIFYUSER:
                                            data.setValue(userInfo.getUserId());
                                            break;
                    ...
                    switch (jnpfKeyType) {
                        ...
                        case JnpfKeyConsts.CURRPOSITION:
                            if (userInfo.getPositionIds() != null && userInfo.getPositionIds().length > 0) {
                                String PositionId = userInfo.getPositionIds()[0];
                                for (String s :userInfo.getPositionIds()){
                                    if (s.contains("-")){
                                        PositionId=s;
                                    }
                                }
